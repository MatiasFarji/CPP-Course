...
void fill(std::vector<std::shared_ptr<Test>> &vec, int num) {
    ...
    for (int i = 0; i < num; i++) {
        ...
        std::cin >> input;
        std::cout << std::endl;
        std::shared_ptr<Test> vec_ptr = std::make_shared<Test>(input);
        vec.push_back(std::move(vec_ptr));
    }
};
void display(const std::vector<std::shared_ptr<Test>>&vec) {
    std::cout << "Displaying vector data" << std::endl;
    std::cout << "======================" << std::endl;
    for (std::shared_ptr<Test> shr_ptr : vec) {
        std::cout << shr_ptr->get_data() << std::endl;
    }
    std::cout << "======================" << std::endl;
};
